...
#define INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME(Name,\
    ...) INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME2(Name, INTERNAL_CATCH_REMOVE_PARENS(\
      __VA_ARGS__))
#else
...
#define INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME(Name, \
    ...) INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME1(Name, \
    INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_REMOVE_PARENS(__VA_ARGS__)))
#endif
...
struct AutoReg : NonCopyable
{
  ...
  static void TestName()
    #define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(TestName, \
    ClassName, \
    ...)    \
  namespace {                                                                                  \
  template<typename TestType>                                                             \
...
///////////////////////////////////////////////////////////////////////////////
  ...
  INTERNAL_CATCH_TESTCASE2(INTERNAL_CATCH_UNIQUE_NAME( \
      ____C_A_T_C_H____T_E_S_T____), __VA_ARGS__)
...
///////////////////////////////////////////////////////////////////////////////
  ...
  struct TestName{\
    ...
    TestName(Ts... names) {\
      CATCH_INTERNAL_CHECK_UNIQUE_TYPES(CATCH_REC_LIST(\
      ...
      (void)expander{(Catch::AutoReg(Catch::makeTestInvoker(&TestFunc<Types>), \
        ...
        Catch::NameAndTags{ names, Tags }), 0)... }; /* NOLINT */ \
    }\
  };\
...
#if defined(CATCH_CPP17_OR_GREATER)
#define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...) static_assert(\
...
#define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...) static_assert(\
  ...
  INTERNAL_CATCH_TEMPLATE_TEST_CASE_2(INTERNAL_CATCH_UNIQUE_NAME( \
      ...
      ____C_A_T_C_H____T_E_M_P_L_A_T_E____T_E_S_T____F_U_N_C____), Name, Tags, \
    __VA_ARGS__)
#else
    #define INTERNAL_CATCH_TEMPLATE_TEST_CASE(Name, Tags, ...) \
  ...
  static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {\
      TestName<CATCH_REC_LIST(INTERNAL_CATCH_REMOVE_PARENS, __VA_ARGS__)>(CATCH_REC_LIST_UD(\
  ...
  INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_2(INTERNAL_CATCH_UNIQUE_NAME( \
      ...
      ____C_A_T_C_H____T_E_M_P_L_A_T_E____T_E_S_T____), ClassName, Name, Tags, \
    __VA_ARGS__)
#else
    #define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD(ClassName, Name, Tags, ...) \
...
#define INTERNAL_CATCH_MSG(macroName, messageType, resultDisposition, ...) \
  do { \
    ...
    catchAssertionHandler.handleMessage(messageType, \
      ( Catch::MessageStream() << __VA_ARGS__ + \
...
#define INTERNAL_CATCH_SECTION(...) \
  ...
  if (Catch::Section const& \
    INTERNAL_CATCH_UNIQUE_NAME(catch_internal_Section) = \
...
auto random(T const& first, T const& last) -> Generator<T>
{
  ...
  return Generator<T>(size,
      pf::make_unique<GeneratorRandomiser<T>>(std::move(gen), size) );
}
template<typename T>
...
{
  return values<std::tuple<Ts...>>(std::forward<std::initializer_list<std::tuple<Ts...>>>(
        tuples) );
}
...
#define GENERATE(...) \
  Catch::Generators::generate(CATCH_INTERNAL_LINEINFO, [] { using namespace Catch::Generators; return makeGenerators( \
        __VA_ARGS__); })
...
private:
  ...
  void addPattern()
  {
    ...
    for (std::size_t i = 0; i < m_escapeChars.size(); ++i)
      token = token.substr(0, m_escapeChars[i]-m_start-i) + token.substr(
        m_escapeChars[i]-m_start-i+1);
    m_escapeChars.clear();
...
TempFile::TempFile()
{
  ...
  if (fopen_s(&m_file, m_buffer, "w"))
  {
    ...
    CATCH_RUNTIME_ERROR(
        "Coul dnot open the temp file: '" << m_buffer << "' because: " <<
        buffer);
  }
}
#else
...
IGeneratorTracker&
{
  ...
  GeneratorTracker& tracker = GeneratorTracker::acquire(m_trackerContext, TestCaseTracking::NameAndLocation(
        "generator",
        lineInfo) );
...
int Session::applyCommandLine(int argc, wchar_t const* const* argv)
{
  ...
  {
    int bufSize = WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, NULL, 0, NULL,
        NULL);
...
void ConsoleReporter::benchmarkStarting(BenchmarkInfo const& info)
{
  ...
  auto nameCol =
    Column(info.name).width(static_cast<std::size_t>( m_tablePrinter->
      columnInfos()[0].width - 2 ) );
...
#define CATCH_TEMPLATE_TEST_CASE(...) \
  INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(INTERNAL_CATCH_UNIQUE_NAME( \
      ____C_A_T_C_H____T_E_M_P_L_A_T_E____T_E_S_T____) )
#define CATCH_TEMPLATE_TEST_CASE_METHOD(className, \
    ...) INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(INTERNAL_CATCH_UNIQUE_NAME( \
      ____C_A_T_C_H____T_E_M_P_L_A_T_E____T_E_S_T____), className)
#else
